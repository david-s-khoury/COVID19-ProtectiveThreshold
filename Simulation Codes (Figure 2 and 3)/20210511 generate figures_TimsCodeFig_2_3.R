...
# Figure 3D --------------------------------------------------------------------------------------------------
startingTitre <- c(1/4, 1/2, 1, 2, 4) # set starting titres
targetTimes<-365*c(1,1.5,2) # The times after which the reduced decay rate is in effect
...
# Set up data frame of results for ggplot
Results <- NULL
...
# setting the tick marks for ggplot
...
# Plot of neutralisation titre versus time.
...
# Figure 3E ------------------------------------------------------------------------------------
mild_ef <- c(70,80,90,95) # Initial mild efficacy's
targetTimes<-365*c(1,1.5,2) # The times after which the reduced decay rate is in effect
...
# Models the decay of efficacy with time
Results <- NULL
for (i in 1:length(mild_ef)){ # Loops through mild efficacies
  for (j in 1:length(targetTimes)) { # Loops through times after which final decay rate is in effect
  mef <- mild_ef[i]
  tragTime<-targetTimes[j]
  
  decay_of_mean_w_time <- log(myfun_titre(1,tragTime)) # Models the decay with time using previous function
  
  # Calculates the percentage uninfected using logistic model for mild infection 
  ...
  # Calculates the percentage uninfected using logistic model for severe infection
...
# Restructures the data for plotting in ggplot
